...
public class ReflectionSecondPage {
	...
	protected void setting() {
		...
		showHistory = (ImageView) edit_thinking_layout.findViewById(R.id.show_text_list2);
		thinking_text = (EditText) edit_thinking_layout.findViewById(R.id.edit_thinking_content2);

		historyListView = (ListView) edit_thinking_layout.findViewById(R.id.listview_page2);
		...
		NoteAdd noteAdd = db.getNoteAddByRelationKey(nowKey);

		if(noteAdd == null)
			dropEditText = new DropEditText(historyListView, showHistory, thinking_text, 4, preAddNote.getThisItems(), context);
		else
			dropEditText = new DropEditText(historyListView, showHistory, thinking_text, 4, noteAdd.getItems(), context);
	...
	class EndOnClickListener implements View.OnClickListener{
		public void onClick(View v){
			...
			if(!sameDay)
			{
				...
				AddScore nowScore = new AddScore(System.currentTimeMillis(), 1, 0, "填寫反思", 0, AddScore.REFLECTION);
