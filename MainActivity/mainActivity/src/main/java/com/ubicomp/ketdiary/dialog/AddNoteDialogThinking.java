...
public class AddNoteDialogThinking implements ChooseItemCaller{
	...
	public AddNoteDialogThinking(TestQuestionCaller2 testQuestionCaller, RelativeLayout mainLayout, Activity activity,
								 AddNoteDialog2 addNoteDialogPre, boolean isUpdate, int relationKey, int item){
	...
	protected void setting() {
		...
		showHistory = (ImageView) description_thinking_layout.findViewById(R.id.show_text_list2);
		thinking_text = (EditText) description_thinking_layout.findViewById(R.id.edit_thinking_content2);

		historyListView = (ListView) description_thinking_layout.findViewById(R.id.listview_page2);
		...
		dropEditText = new DropEditText(historyListView, showHistory, thinking_text, 2, items, context);
	...
	class EndOnClickListener implements View.OnClickListener{
		public void onClick(View v){
			...
			if(!sameDay && !sameDay2)
			{
				...
				AddScore nowScore = new AddScore(System.currentTimeMillis(), 2, 0, "填寫事件與想法", 0, AddScore.NOTE+AddScore.THINKING);
			...
			if(sameDay && !sameDay2)
			{
				CustomToast.generateToast(R.string.add_note_with_thinking, 1);
				AddScore preScore = db.getLastestAddScore();
				AddScore nowScore = new AddScore(System.currentTimeMillis(), 1, 0, "填寫想法", 0, AddScore.THINKING);
				db.insertAddScore(nowScore);
			}
