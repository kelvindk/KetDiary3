...
public class ReflectionFirstPage implements ChooseItemCaller{
	...
	protected void setting() {
		...
		showHistory = (ImageView) description_reflection_layout.findViewById(R.id.show_text_list1);
		thinking_text = (EditText) description_reflection_layout.findViewById(R.id.edit_thinking_content);

		historyListView = (ListView) description_reflection_layout.findViewById(R.id.listview_page1);
		...
		if(noteAdd == null)
			dropEditText = new DropEditText(historyListView, showHistory, thinking_text, 3, preAddNote.getThisItems(), context);
		else
			dropEditText = new DropEditText(historyListView, showHistory, thinking_text, 3, noteAdd.getItems(), context);
